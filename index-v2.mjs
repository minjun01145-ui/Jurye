...
// 🚀 [공개수업 안정성 우선 패치]
// 학교 와이파이/크롬북/프록시 환경에서 Firestore 실시간 Listen 연결이 흔들릴 때,
// 빠른 기본 연결보다 조금 덜 빠르더라도 더 안정적인 long-polling을 강제로 사용합니다.
...
async function loadCharacterList() {
  try {
    ...
    if (availableCharacters.length > 0) {
      ...
      if (avatarDisp) {
        avatarDisp.innerHTML = `<img src="char/${availableCharacters[0]}/stand1_0.png" class="anim-avatar profile-character-image" data-char-id="${availableCharacters[0]}" alt="기본 캐릭터" />`;
      }
...
const ENTRY_SCREEN_IDS = new Set(["auth-screen", "login-screen"]);
const APP_SCREEN_IDS = new Set([
  "lobby-mode-screen", "set-select-screen", "menu-screen", "char-shop-screen",
  "multi-lobby-screen", "teacher-lobby-screen", "admin-main-screen",
  "admin-feedback-screen", "admin-student-screen", "admin-set-list-screen",
  "admin-set-edit-screen", "list-screen", "fc-option-screen", "time-option-screen",
  "result-screen", "feedback-screen", "ranking-screen", "highfive-result-screen"
]);

const BREADCRUMB_LABELS = {
  "auth-screen": "로그인 화면",
  "login-screen": "프로필 설정",
  "lobby-mode-screen": "시작 화면",
  "set-select-screen": "혼자하기 · 학습 세트 선택",
  "menu-screen": "게임 선택",
  "char-shop-screen": "캐릭터 상점",
  "multi-lobby-screen": "멀티플레이 대기실",
  "teacher-lobby-screen": "교사용 멀티플레이",
  "admin-main-screen": "관리자",
  "admin-feedback-screen": "학생 의견",
  "admin-student-screen": "학생 관리",
  "admin-set-list-screen": "세트 관리",
  "admin-set-edit-screen": "세트 편집",
  "list-screen": "학습 목록",
  "fc-option-screen": "깜박이 옵션",
  "time-option-screen": "플레이 시간",
  "result-screen": "결과",
  "feedback-screen": "의견 남기기",
  "ranking-screen": "랭킹",
  "highfive-result-screen": "조편성 결과"
};

function canOpenStartScreen() {
  return Boolean(currentUser?.nickname);
}

function getBreadcrumbItems(screenId) {
  const loginRoot = { label: "로그인 화면" };
  const start = {
    label: "시작 화면",
    target: canOpenStartScreen() ? "lobby-mode-screen" : null
  };
  const soloSet = {
    label: "혼자하기 · 학습 세트 선택",
    target: "set-select-screen"
  };
  const gameMenu = {
    label: "게임 선택",
    target: "menu-screen"
  ...
  switch (screenId) {
    ...
    case "lobby-mode-screen":
      return [
        loginRoot,
        { label: "시작 화면" }
      ];

    case "set-select-screen":
      return [
        loginRoot,
        start,
        { label: "혼자하기 · 학습 세트 선택" }
    ...
    case "menu-screen":
      return [
        ...
        start,
        soloSet,
        { label: currentSetTitle ? `게임 선택 · ${currentSetTitle}` : "게임 선택" }
      ];
    ...
    case "feedback-screen":
      if (window.feedbackReturnScreen === "result-screen") {
        return [
          loginRoot,
          start,
          soloSet,
          gameMenu,
          { label: "결과", target: "result-screen" },
          { label: "의견 남기기" }
        ];
      }
      return [
        loginRoot,
        start,
        { label: "멀티플레이 대기실", target: "multi-lobby-screen" },
        { label: "의견 남기기" }
      ];

    case "char-shop-screen": {
      const items = [loginRoot];
      if (canOpenStartScreen()) items.push(start);
      items.push({ label: "캐릭터 상점" });
      return items;
    }

    case "admin-main-screen":
      return [
        loginRoot,
        { label: "관리자" }
      ];

    case "admin-feedback-screen":
      return [
        loginRoot,
        { label: "관리자", target: "admin-main-screen" },
        { label: "학생 의견" }
      ];

    case "admin-student-screen":
      return [
        loginRoot,
        { label: "관리자", target: "admin-main-screen" },
        { label: "학생 관리" }
    ...
    case "admin-set-edit-screen":
      return [
        loginRoot,
        { label: "관리자", target: "admin-main-screen" },
        { label: "세트 관리", target: "admin-set-list-screen" },
        { label: "세트 편집" }
      ];

    case "highfive-result-screen":
      return [
        loginRoot,
        start,
        { label: "멀티플레이", target: "multi-lobby-screen" },
        { label: "조편성 결과" }
      ];

    default: {
      const label = BREADCRUMB_LABELS[screenId];
      return label ? [loginRoot, start, { label }] : [];
    }
  }
}

function renderAppBreadcrumb(screenId) {
  const host = document.getElementById("app-breadcrumb");
  if (!host) return;

  const items = getBreadcrumbItems(screenId);
  ...
  items.forEach((item, index) => {
    ...
    if (item.target && !isCurrent) {
      element = document.createElement("button");
      element.type = "button";
      element.className = "breadcrumb-item breadcrumb-link";
      element.dataset.breadcrumbTarget = item.target;
...
function updateAppShellHeader(screenId) {
  const back = document.getElementById("app-back-btn");
  if (back) back.hidden = screenId === "lobby-mode-screen" || ENTRY_SCREEN_IDS.has(screenId);
  renderAppBreadcrumb(screenId);
  updatePlayerStatusBadge();
}

function getScreenDisplayMode(screen) {
  if (!screen) return "none";
  if (screen.classList.contains("entry-screen")) return "block";
  if (screen.classList.contains("app-screen")) return "block";
  return "flex";
}
...
function showScreen(screenId) {
  ...
  const entryCredits = document.getElementById("entry-credits");
  ...
  document.body.classList.remove("entry-open", "app-mode", "immersive-mode");

  if (ENTRY_SCREEN_IDS.has(screenId)) {
    ...
    const home = document.getElementById("lobby-mode-screen");
    if (home) {
      home.hidden = false;
    ...
    if (entryOverlay) {
      ...
      entryOverlay.classList.add("is-open");
    }

    if (entryCredits) entryCredits.hidden = screenId !== "auth-screen";

    const entryScreen = document.getElementById(screenId);
    if (entryScreen) {
      entryScreen.hidden = false;
      entryScreen.style.display = getScreenDisplayMode(entryScreen);
      entryScreen.classList.add("active");
    }
  } else if (APP_SCREEN_IDS.has(screenId)) {
    document.body.classList.add("app-mode");
    if (entryOverlay) {
      entryOverlay.hidden = true;
      entryOverlay.classList.remove("is-open");
    }
    if (appShell) {
      appShell.hidden = false;
    ...
    if (screen) {
      screen.hidden = false;
      screen.style.display = getScreenDisplayMode(screen);
  ...
  updateAppShellHeader(screenId);

  const gameScreens = ["flashcard-screen", "memory-screen", "speed-match-screen", "speed-screen", "fishing-screen", "chunk-screen"];
  const container = document.getElementById("walking-emoji-container");
  if (container) container.style.opacity = gameScreens.includes(screenId) ? "0" : "1";

  const lobbyShopModal = document.getElementById("lobby-inline-shop-container");
  if (lobbyShopModal && screenId !== "multi-lobby-screen") lobbyShopModal.style.display = "none";
  if (screenId === "multi-lobby-screen") ensureLobbyShopButtonAndModal();
}

window.jrShowScreen = showScreen;
window.jrGetCurrentScreen = () => currentScreenId;

function bindClick(id, callback) {
  return bindClickCore(id, callback);
}

bindClick("app-home-btn", () => {
  playSound("click");
  showScreen(canOpenStartScreen() ? "lobby-mode-screen" : "login-screen");
});

document.getElementById("app-breadcrumb")?.addEventListener("click", (event) => {
  const target = event.target.closest("[data-breadcrumb-target]");
  if (!target) return;

  const screenId = target.dataset.breadcrumbTarget;
  if (!screenId) return;
...
bindClick("app-back-btn", () => {
  playSound("click");
  const current = window.jrGetCurrentScreen ? window.jrGetCurrentScreen() : "";
  const backMap = {
    "set-select-screen": "lobby-mode-screen",
    "menu-screen": "set-select-screen",
    "char-shop-screen": "login-screen",
    "multi-lobby-screen": "lobby-mode-screen",
    "teacher-lobby-screen": "lobby-mode-screen",
    "list-screen": "menu-screen",
    "fc-option-screen": "menu-screen",
    "time-option-screen": "menu-screen",
    "result-screen": "menu-screen",
    "feedback-screen": "multi-lobby-screen",
    "ranking-screen": "menu-screen",
    "admin-main-screen": "auth-screen",
    "admin-feedback-screen": "admin-main-screen",
    "admin-student-screen": "admin-main-screen",
    "admin-set-list-screen": "admin-main-screen",
    "admin-set-edit-screen": "admin-set-list-screen"
  };
  showScreen(backMap[current] || "lobby-mode-screen");
});

bindClick("app-profile-btn", () => {
  playSound("click");
...
function updatePlayerStatusBadge() {
  const jr = Number(currentUser?.jr || 0);
  const jrEls = [
    document.getElementById("app-jr-display"),
    document.getElementById("shop-jr-display"),
    document.getElementById("user-jr-display")
  ];
  jrEls.forEach((el) => { if (el) el.textContent = jr.toLocaleString("ko-KR"); });

  const avatar = document.getElementById("app-profile-avatar");
  const charFolder = currentUser?.character || availableCharacters[0];
  if (avatar && charFolder) {
    avatar.src = `char/${charFolder}/stand1_0.png`;
    avatar.dataset.charId = charFolder;
...
function updateLoadingProgress(percent, title, detail, showWarning = false) {
  const titleEl = document.getElementById("loading-title") || document.querySelector("#loading-screen h1, #loading-screen h2");
  ...
  if (detailEl && detail !== undefined) detailEl.innerText = detail;
...
function renderCharShopList() {
  ...
  availableCharacters.forEach(charFolder => {
    ...
    const lockText = isOwned ? `<span class="state-owned">보유 중</span>` : `<span class="state-locked">2,000 JR</span>`;

    const card = document.createElement("button");
    card.type = "button";
    card.className = "shop-char-card";
    card.innerHTML = `
      <span class="shop-char-visual"><img src="char/${charFolder}/stand1_0.png" class="anim-avatar" data-char-id="${charFolder}" alt="${charName}"></span>
      <strong class="shop-char-name">${charName}</strong>
      <span class="shop-char-creator">만든이: ${creatorName}</span>
      <span class="shop-char-state">${lockText}</span>
    `;
...
function renderLobbyGrid(players, showId = false) {
  ...
  players.forEach((player) => {
    ...
    characterWrap.className = "lobby-player-character";

    const bubble = document.createElement("div");
    bubble.id = `bubble-${player.stdId}`;
    bubble.className = "chat-bubble lobby-chat-bubble";

    const charFolder = player.character || availableCharacters[0] || "기본0(민준쌤)";
    const avatar = document.createElement("img");
    avatar.src = `char/${charFolder}/stand1_0.png`;
    avatar.className = "anim-avatar lobby-player-avatar";
    avatar.dataset.charId = charFolder;
    avatar.alt = `${player.nickname || "학생"} 캐릭터`;

    const name = document.createElement("span");
    name.className = "lobby-player-name";
    name.textContent = player.nickname || player.realName || player.stdId || "이름 없음";

    if (showId) {
      const identity = document.createElement("span");
      identity.className = "lobby-player-id";
      identity.textContent = `${player.stdId || ""} ${player.realName || ""}`.trim();
      name.appendChild(identity);
    }

    characterWrap.append(bubble, avatar);
    card.append(characterWrap, name);
...
function renderTeacherVisualLobby(players) {
  ...
  if (!players || players.length === 0) {
    ...
    empty.innerHTML = `
      <span>👀</span>
      <strong>학생 접속을 기다리는 중</strong>
      <small>학생들이 대기실에 들어오면 이곳에 표시됩니다.</small>
    `;
  ...
  if (currentGroupingActive) {
    let maxGroup = 0;
    players.forEach((player) => {
      if (player.groupId > maxGroup) maxGroup = player.groupId;
    });

    for (let groupId = 1; groupId <= maxGroup; groupId++) {
      const groupPlayers = players.filter((player) => player.groupId === groupId);
      if (groupPlayers.length === 0) continue;

      const groupCard = document.createElement("section");
      groupCard.className = "teacher-group-card";

      const heading = document.createElement("h3");
      heading.textContent = `${groupId}조 · ${groupPlayers.length}명`;

      const groupGrid = document.createElement("div");
      groupGrid.className = "teacher-group-grid";
      groupPlayers.forEach((player) => groupGrid.appendChild(createTeacherPlayerCard(player)));

      groupCard.append(heading, groupGrid);
      container.appendChild(groupCard);
    }
    return;
  }

  players.forEach((player) => container.appendChild(createTeacherPlayerCard(player)));
}
...
bindClick("teacher-toggle-name-btn", () => {
  ...
  const button = document.getElementById("teacher-toggle-name-btn");
  if (button) {
    button.textContent = isTeacherNameHidden
      ? "👀 학번·이름 보이기"
      : "🙈 학번·이름 숨기기";
    button.classList.toggle("is-revealing", isTeacherNameHidden);
  }
...
function createTeacherPlayerCard(player) {
  const card = document.createElement("article");
  card.className = "teacher-player-card";
  card.title = "클릭하면 이 학생을 대기실에서 내보냅니다.";

  card.addEventListener("click", () => {
    if (typeof window.kickPlayer === "function") {
      window.kickPlayer(player.docId, player.nickname);
    }
  });

  const bubble = document.createElement("div");
  bubble.id = `teacher-bubble-${player.stdId}`;
  bubble.className = "chat-bubble teacher-chat-bubble";

  const charFolder = player.character || availableCharacters[0] || "기본0(민준쌤)";
  const avatar = document.createElement("img");
  avatar.src = `char/${charFolder}/stand1_0.png`;
  avatar.className = "anim-avatar teacher-player-avatar";
  avatar.dataset.charId = charFolder;
  avatar.alt = `${player.nickname || "학생"} 캐릭터`;

  const name = document.createElement("span");
  name.className = "teacher-player-name";
  name.textContent = player.nickname || player.realName || player.stdId || "이름 없음";

  if (!isTeacherNameHidden) {
    const identity = document.createElement("span");
    identity.className = "teacher-player-identity";
    identity.textContent = `${player.stdId || ""} ${player.realName || ""}`.trim();
    name.appendChild(identity);
  }

  card.append(bubble, avatar, name);
  return card;
}
...
async function loadAllFromDB() {
  ...
  updateLoadingProgress(5, "데이터를 불러오는 중입니다.", "서버 연결 준비 중...");

  for (let i = 0; i < maxRetries; i++) {
    try {
      const attemptText = `서버 연결 시도 중... (${i + 1}/${maxRetries})`;
      updateLoadingProgress(10 + i * 5, "데이터를 불러오는 중입니다.", attemptText, i > 0);

      // 1. 단어 세트 불러오기
      updateLoadingProgress(25, "데이터를 불러오는 중입니다.", "단어/문제 세트를 불러오는 중...");
      const setSnap = await withTimeout(getDoc(doc(db, "gameData", "wordSets")), 8000, "단어 세트");
      if (setSnap.exists()) {
      ...
      // 2. 학생 명단 불러오기
      updateLoadingProgress(55, "데이터를 불러오는 중입니다.", "학생 명단을 불러오는 중...");
      const stdSnap = await withTimeout(getDoc(doc(db, "gameData", "students")), 8000, "학생 명단");
      if (stdSnap.exists()) {
        studentList = stdSnap.data().students || [];
        localStorage.setItem("backup_studentList", JSON.stringify(studentList)); 
      }

      updateLoadingProgress(85, "데이터를 불러오는 중입니다.", "불러온 데이터를 정리하는 중...");
      success = true; 
      break; 

    } catch (error) { 
      console.warn(`DB 연결 실패 또는 지연 (재시도 ${i+1}/${maxRetries}):`, error);
      updateLoadingProgress(15 + i * 20, "서버 응답이 늦습니다.", `재시도 준비 중... (${i + 1}/${maxRetries})`, true);
      await new Promise(resolve => setTimeout(resolve, 1000));
    }
  }

  if (!success) {
    console.error("서버 통신 실패, 내장 백업 데이터를 확인합니다.");
    updateLoadingProgress(70, "서버 응답이 불안정합니다.", "이 기기에 저장된 백업 데이터를 확인하는 중...", true);
    ...
    if (backupSets && backupStds) {
      try {
        ...
        success = true; 
        updateLoadingProgress(90, "백업 데이터로 복구했습니다.", "로그인 화면으로 이동합니다...");
      } catch(e) {
...
if (success) {
    ...
    updateLoadingProgress(100, "서버 연결 실패 ㅠㅠ", "학교 네트워크 또는 Firebase 응답이 원활하지 않습니다.", true);
...
if (loadingRetryBtn) loadingRetryBtn.addEventListener("click", () => location.reload());
...
bindClick("auth-btn", async () => {
  ...
  if (matchedStudent) {
    ...
    // 로딩 화면 띄우기
    showScreen("loading-screen"); 
    updateLoadingProgress(25, "프로필 정보를 불러오는 중...", "학생 정보와 보상을 확인하고 있습니다.");
...
function renderSetSelectList() {
  ...
  const normalSets = wordSets.filter((set) => !set.isCustomSet);
  normalSets.forEach((set, index) => {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "set-card";
    btn.innerHTML = `
      <span class="set-card-index">${String(index + 1).padStart(2, "0")}</span>
      <span class="set-card-title">${set.title}</span>
      <span class="set-card-meta">${set.words.length}개 항목</span>
      <span class="set-card-arrow">→</span>
    `;
...
bindClick("set-select-back-btn", () => { playSound("click"); showScreen("lobby-mode-screen"); });
...
bindClick("admin-reset-jr-btn", async () => {
    ...
    if (confirm("정말 모든 학생의 JR을 0으로 만들고, 구매한 캐릭터를 모두 몰수하시겠습니까?\n(새 학기나 대규모 이벤트 리셋용)")) {
        ...
        if (pwd === "1234") {
            showScreen("loading-screen");
            updateLoadingProgress(25, "전체 학생 재화 초기화 중...", "학생 데이터를 안전하게 갱신하고 있습니다.");
            try {
...
bindClick("menu-list-btn", () => {
  ...
  const wordToggle = document.getElementById("toggle-word-btn");
  const meanToggle = document.getElementById("toggle-mean-btn");

  if (wordToggle) {
    wordToggle.classList.remove("is-active");
    const label = wordToggle.querySelector("strong");
    if (label) label.textContent = "영어 가리기";
  }
  if (meanToggle) {
    meanToggle.classList.remove("is-active");
    const label = meanToggle.querySelector("strong");
    if (label) label.textContent = "뜻 가리기";
  }
...
bindClick("toggle-word-btn", () => {
  ...
  const button = document.getElementById("toggle-word-btn");
  button?.classList.toggle("is-active", isWordHidden);
  const label = button?.querySelector("strong");
  if (label) label.textContent = isWordHidden ? "영어 보이기" : "영어 가리기";

  document.querySelectorAll(".word-text-col span").forEach((element) => {
    element.classList.toggle("hidden-text", isWordHidden);
  });
});
...
bindClick("toggle-mean-btn", () => {
  ...
  const button = document.getElementById("toggle-mean-btn");
  button?.classList.toggle("is-active", isMeanHidden);
  const label = button?.querySelector("strong");
  if (label) label.textContent = isMeanHidden ? "뜻 보이기" : "뜻 가리기";

  document.querySelectorAll(".mean-text-col span").forEach((element) => {
    element.classList.toggle("hidden-text", isMeanHidden);
  });
});
...
function renderWordList() {
  ...
  if (title) title.textContent = currentSetTitle || "학습 목록";
  if (count) count.textContent = String(wordList.length);
  if (!container) return;
  ...
  if (wordList.length === 0) {
    ...
    empty.textContent = "이 세트에는 표시할 학습 항목이 없습니다.";
    container.appendChild(empty);
    return;
  }
...
let earnedJR = Math.floor(currentUser.score / 100);
  ...
  if (myLobbyDocId && (currentGameMode === "speed" || currentGameMode === "speed-match" || currentGameMode === "chunk" || currentGameMode === "custom_infinite")) {
    showScreen("loading-screen");
    updateLoadingProgress(45, "다른 친구들의 점수를 집계 중입니다...", "랭킹 데이터를 불러오고 있습니다.");
...
async function showRankings(tab, mode = currentRankingMode) {
  ...
  const modeNames = {
    ...
    "fish": "🎣 이모지 낚시",
    "chunk": "🧾 문장 해석 게임",
    "custom_infinite": "♾️ 무한 퀴즈",
    "boss_raid": "👹 보스 레이드"
  };

  const modeTitle = document.getElementById("ranking-mode-title");
  const scoreMessage = document.getElementById("ranking-msg");
  const encourage = document.getElementById("ranking-encourage");
  const list = document.getElementById("ranking-list");

  if (modeTitle) modeTitle.textContent = modeNames[currentRankingMode] || "순위표";
  if (scoreMessage) scoreMessage.textContent = `${currentUser.nickname || "플레이어"} · ${currentUser.score || 0}점`;
  ...
  if (encourage) encourage.textContent = quotes[Math.floor(Math.random() * quotes.length)];

  if (!list) return;
  list.innerHTML = '<div class="ranking-loading">순위를 불러오는 중입니다.</div>';
  ...
  try {
    ...
    let allScores = [];
    snapshot.forEach((scoreDoc) => allScores.push(scoreDoc.data()));

    let filtered = allScores.filter(
      (score) => score.mode === currentRankingMode && score.setId === currentSetId
    );
    ...
    if (tab === "today") {
      filtered = filtered.filter((score) => score.timestamp >= todayStart);
    } else if (tab === "class") {
      filtered = filtered.filter((score) => score.classId === currentUser.classId);
    }

    const uniqueTop = {};
    filtered.forEach((score) => {
      if (!uniqueTop[score.stdId] || uniqueTop[score.stdId].score < score.score) {
        uniqueTop[score.stdId] = score;
      }
    });

    const sorted = Object.values(uniqueTop).sort((a, b) => b.score - a.score);
    ...
    sorted.forEach((score, index) => {
      ...
      name.textContent = score.nickname || score.realName || score.stdId || "이름 없음";

      const value = document.createElement("span");
      value.className = "rank-score";
      value.textContent = `${score.score || 0}점`;

      person.append(medal, name);
      item.append(person, value);
      list.appendChild(item);
    });

    fireConfetti();
  } catch (error) {
    list.innerHTML = '<div class="ranking-error">데이터를 불러오지 못했습니다.</div>';
    console.error(error);
  }
}
...
// V2 홈 화면은 공통 app-back-btn을 사용하므로 별도 lobby-mode-back-btn은 두지 않습니다.
...
async function rescueJoinCurrentGameIfPlaying(reason = "direct-check") {
  ...
  try {
    ...
    if (room.endTime && room.gameMode !== "highfive" && Date.now() > Number(room.endTime) + 2000) {
...
async function enterMultiLobbyAsStudent() {
  ...
  try {
    ...
    try {
      ...
      const ghostSnap = await getDocs(collection(db, "lobbyUsers"));
      ...
      ghostSnap.forEach(d => {
        ...
        if (p.stdId === currentUser.stdId) {
          ...
          ghostDeletePromises.push(deleteDoc(doc(db, "lobbyUsers", d.id)).catch(e => e));
        }
      });
    ...
    const docRef = await addDoc(collection(db, "lobbyUsers"), newLobbyData);
...
if (currentGroupingActive && currentMultiRoomGroupPlayMode === "one-player" && !["highfive", "showcase"].includes(room.gameMode)) {
        ...
        window.isMultiGameActive = false;
        toggleStudentLobbyListeners(true); 
          // 🚑 [하이파이브 긴급 안정화] 학생이 대기실에 있는 동안 1초마다 현재 방 상태를 직접 확인합니다.
  // onSnapshot 실시간 신호가 늦거나 누락되어도 highfive가 시작되어 있으면 직접 끌고 옵니다.
  if (window.highFivePullerInterval) {
    clearInterval(window.highFivePullerInterval);
  }

  window.highFivePullerInterval = setInterval(async () => {
    try {
      ...
      const isInUsefulScreen =
        (lobbyScreen && lobbyScreen.classList.contains("active")) ||
        (hfScreen && hfScreen.classList.contains("active")) ||
        (hfResultScreen && hfResultScreen.classList.contains("active"));

      if (!isInUsefulScreen) return;

      const roomSnap = await getDoc(doc(db, "gameData", "multiRoom"));
      if (!roomSnap.exists()) return;

      const room = roomSnap.data();
      forceStartHighFiveIfNeeded(room, "polling");

    } catch(e) {
      // 1초 감시 장치는 실패해도 게임 전체를 멈추면 안 됩니다.
      console.warn("하이파이브 직접 확인 실패", e);
    }
  }, 1000);
...
let alertMsg = currentGameMode === "showcase" ? "✨ 쇼케이스가 종료되었습니다. 대기실로 이동합니다." : "👑 선생님이 활동을 종료하셨습니다!\n실시간 대기실로 이동합니다.";
  ...
  setTimeout(() => {
    rescueJoinCurrentGameIfPlaying("student-enter-direct-check").catch(e => {
...
function updateTeacherMenuVisibility() {
    ...
    const legacyCustomSetField = document.querySelector("#teacher-custom-options-container .teacher-legacy-custom-set");
    if (legacyCustomSetField) legacyCustomSetField.style.display = "none";
...
function setupTeacherLiveMatch(endTime) {
    ...
    if (abortBtn) {
        abortBtn.classList.remove("is-return-action");
...
if (remaining <= 0) {
            ...
            if (abortBtn) {
                ...
                abortBtn.classList.add("is-return-action");
            }
        }
    }, 500);
}
...
function cleanupTeacherLiveMatch() {
    ...
    document.getElementById("teacher-visual-lobby-grid").style.display = "grid";
    ...
    if (currentGameMode === "create") {
        if (currentGroupingActive && window.teacherGroupPlayMode) {
            ...
            renderList = Object.values(groupData).map(g => {
                ...
                return {
                    renderId: `group-${g.id}`,
                    title: `<span class="leaderboard-group-name">${g.id}조</span>`,
                    subtitle: `<span class="leaderboard-subtitle">(조별 출제)</span><span class="leaderboard-members">👤 ${getGroupMembersText(g.id)}</span>`,
                    scoreText: `${g.createdCount} / ${target} 완료`,
                    isDone: g.isSubmitted,
                    icon: `<span class="leaderboard-group-icon">👥</span>`
                }
            });
        } else {
            renderList = players.map(p => {
                const charFolder = p.character || availableCharacters[0] || "기본0(민준쌤)";
                const nameHtml = isTeacherNameHidden ? "" : ` <span class="leaderboard-identity">(${p.stdId} ${p.realName || ''})</span>`;
                return {
                    renderId: `user-${p.stdId}`,
                    title: p.nickname, subtitle: nameHtml,
                    scoreText: `${p.createdCount || 0} / ${p.targetCount || 1} 완료`,
                    isDone: p.isSubmitted,
                    icon: `<img src="char/${charFolder}/stand1_0.png" class="anim-avatar live-rank-avatar" data-char-id="${charFolder}" alt="">`
                };
            });
        }
        ...
        renderList.forEach((p, index) => {
            ...
            if (!row) {
                ...
                row.innerHTML = `<div class="live-rank-badge"></div><div class="live-rank-user"></div><div class="live-rank-items"></div><div class="live-rank-score"></div>`;
                board.appendChild(row);
            }
            row.style.top = topPos + "px"; row.className = `live-rank-item ${rankClass} ${p.isDone ? "is-complete" : "is-pending"}`;
            ...
            row.querySelector(".live-rank-score").style.color = "";
        });
        return; 
    }
...
renderList = Object.values(groupData).map(g => ({
    renderId: `group-${g.id}`,
    title: `<span class="leaderboard-group-name">${g.id}조</span>`,
    subtitle: `<span class="leaderboard-subtitle">(조별 합산 점수)</span><span class="leaderboard-members">👤 ${getGroupMembersText(g.id)}</span>`,
    score: g.score || 0,
    items: g.items || "",
    icon: `<span class="leaderboard-group-icon">👥</span>`
}));
    ...
    sorted.forEach((p, index) => {
        ...
        if (!row) {
            row = document.createElement("div"); row.id = `live-rank-${p.renderId}`; row.className = `live-rank-item`;
            row.innerHTML = `<div class="live-rank-badge"></div><div class="live-rank-user"></div><div class="live-rank-items"></div><div class="live-rank-score">0점</div>`;
            board.appendChild(row);
        }
...
const backBtnIds = [
  ...
  "lobby-exit-btn", "teacher-lobby-exit-btn", 
...
async function processHighFiveResult() {
    showScreen("loading-screen");
    updateLoadingProgress(45, "마음이 통하는 친구들을 찾고 있습니다...", "하이파이브 결과를 계산하고 있습니다.");
    ...
    groups.forEach((group, index) => {
      const groupCard = document.createElement("section");
      groupCard.className = "group-result-card";

      const heading = document.createElement("h3");
      heading.textContent = `${index + 1}조 · ${group.length}명`;
      ...
      group.forEach((member) => {
        ...
        memberCard.className = "group-member";

        const charFolder = member.character || "기본0(민준쌤)";
        const avatar = document.createElement("img");
        avatar.src = `char/${charFolder}/stand1_0.png`;
        avatar.className = "anim-avatar";
        avatar.dataset.charId = charFolder;
        avatar.alt = `${member.nickname || "학생"} 캐릭터`;

        const copy = document.createElement("div");
        const nickname = document.createElement("strong");
        nickname.textContent = member.nickname || member.realName || member.stdId || "이름 없음";

        const identity = document.createElement("small");
        identity.textContent = `${member.stdId || ""} ${member.realName || ""}`.trim();

        copy.append(nickname, identity);
        memberCard.append(avatar, copy);
        memberList.appendChild(memberCard);
      });

      groupCard.append(heading, memberList);
      container.appendChild(groupCard);
    });

    // 교사에게만 확정/취소 버튼 보이기, 학생은 대기 메시지 띄우기
    const playBtn = document.getElementById("hf-result-play-btn");
    if (playBtn) playBtn.style.display = isTeacherMode ? "inline-flex" : "none";

    const homeBtn = document.getElementById("hf-result-home-btn");
    if (homeBtn) homeBtn.style.display = isTeacherMode ? "inline-flex" : "none";
    ...
    if (!studentWaitMsg) {
      ...
      studentWaitMsg.className = "hf-student-wait";
      studentWaitMsg.textContent = "⏳ 선생님의 조편성 확정을 기다리는 중...";
      document.getElementById("hf-result-container")?.appendChild(studentWaitMsg);
    }
    studentWaitMsg.style.display = isTeacherMode ? "none" : "flex";
...
window.processIncomingChat = function(c) {
  ...
  const log = document.getElementById("student-chat-log");
  if (!log) return;
  ...
  author.textContent = `${isTeacher ? "📢 " : ""}${c.nickname || "이름 없음"}`;
...
bindClick("create-submit-btn", async () => {
    ...
    showScreen("loading-screen");
    updateLoadingProgress(60, "제출 및 보상 정산 중...", "문제와 보상 데이터를 저장하고 있습니다.");
