...
function triggerTreasureEvent(callback) {
  ...
  chests.forEach(chest => {
    chest.onclick = () => {
      playSound("click"); 
      chest.style.transform = "scale(1.5)"; chest.style.opacity = "0"; // CSS transition으로 스무스하게
      setTimeout(() => {
...
function resetGameStates() {
  clearInterval(gameTimerInterval); clearInterval(cdInterval); isGamePaused = false; gameScore = 0; globalScoreMultiplier = 1;
