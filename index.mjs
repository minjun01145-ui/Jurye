...
let liveGameStartTimeout = null; // 🌟 10초 연습모드 종료 타이머 (버그 방지용)
...
function animateRoaming(time) {
  ...
  roamingEmojis.forEach(e => {
    ...
    if(!e.isJumping && Math.random() < 0.01) { e.isJumping = true; e.vy = -200; }
    if(e.isJumping) {
      e.yOffset += e.vy * dt; e.vy += 600 * dt; 
...
function showScreen(screenId) { 
  ...
  // 로비, 게임 등일 때는 이모지 숨기기, 메뉴 화면일 때는 켜기
...
async function loadAllFromDB() {
  ...
  try {
    ...
    if(authBtn) { authBtn.innerText = "인증하기"; authBtn.disabled = false; authBtn.classList.add("btn-cyan"); }
    startRoamingEmojis(); // 첫 구동 시 이모지 출발
...
async function enterLobby() {
  ...
  lobbyUnsubChat = onSnapshot(qChat, (snap) => { let msgs = []; snap.forEach(d => msgs.push(d.data())); msgs.reverse(); renderLobbyChat(msgs, "lobby-chat-box"); });
  ...
  liveStateUnsub = onSnapshot(doc(db, "gameData", "liveRoom"), (docSnap) => {
    if(docSnap.exists()) {
      ...
      document.getElementById("lobby-game-info").innerHTML = `모드: ${modeNames[data.mode]||"미정"}<br>시간: ${data.time/60}분<br>단어장: ${setText}`;
...
function renderLobbyChat(msgs, boxId) {
  ...
  box.innerHTML = msgs.map(m => {
    ...
    return `<div class="chat-msg ${isMe ? 'me' : ''}">` + (isMe ? "" : `<span style="color:#555; font-size:12px;">${m.emoji} ${m.nickname}<br></span>`) + `${m.text}</div>`;
...
function startLivePractice(config, targetWords) {
  ...
  let banner = document.getElementById("live-sm-banner"); banner.style.display = "block"; banner.style.background = "#FF003C"; banner.innerText = "💪 [연습] 10초 몸풀기!";
...
function startLiveMainGame(config, targetWords) {
  ...
  let banner = document.getElementById("live-sm-banner"); banner.style.display = "block"; banner.style.background = "#00E5FF"; banner.style.color = "#000"; banner.innerText = "🔥 [본 게임] 랭킹전 시작!";
  gameScore = 0; document.getElementById("sm-score").innerText = `SCORE: 0`;
...
bindClick("admin-live-start-btn", async () => {
  playSound("click");
  const result = await Swal.fire({ title: 'ALL SYSTEMS GO?', text: "학생들을 게임으로 납치(?)합니다!", icon: 'warning', showCancelButton: true, confirmButtonText: 'LAUNCH' });
  ...
  liveGameStartTimeout = setTimeout(async () => { 
    ...
    document.getElementById("admin-live-status-text").innerText = "> STATUS: RANKED_MATCH_ONGOING"; 
  }, 10500); 
});

bindClick("admin-live-end-btn", async () => {
  playSound("click"); const result = await Swal.fire({ title: 'FORCE STOP?', text: "게임을 끝내시겠습니까?", icon: 'warning', showCancelButton: true, confirmButtonText: 'STOP' }); if(!result.isConfirmed) return;
  if(liveGameStartTimeout) clearTimeout(liveGameStartTimeout); // 타이머 누수 완벽 차단!
...
function startExamCountdown(logicCallback) {
  showScreen(currentGameMode === "exam1" ? "exam1-screen" : "exam2-screen"); document.getElementById("top-left-controls").style.display = "flex";
  let count = 3; document.getElementById("huge-countdown-overlay").style.display = "flex"; document.getElementById("huge-text").innerText = count;
...
function renderExam1Cards() {
  const pool = document.getElementById("exam1-pool"); const slots = document.getElementById("exam1-slots"); const submitBtn = document.getElementById("exam1-submit-btn"); pool.innerHTML = ''; slots.innerHTML = '';
...
bindClick("exam1-submit-btn", () => {
  ...
  if (answer === target) { playSound("success"); gameScore += 100; document.getElementById("exam1-score").innerText = `SCORE: ${gameScore}`; setTimeout(() => { examCurrentIndex++; isGamePaused = false; loadExam1Question(); }, 800); } 
  else { playSound("wrong"); gameScore = Math.max(0, gameScore - 50); document.getElementById("exam1-score").innerText = `SCORE: ${gameScore}`; setTimeout(() => { examWords = [...examWords, ...examSlots]; examSlots = []; renderExam1Cards(); isGamePaused = false; }, 800); }
});
...
bindClick("exam2-submit-btn", () => {
  ...
  if (normInput === normTarget && normTarget.length > 0) { playSound("success"); gameScore += 100; document.getElementById("exam2-score").innerText = `SCORE: ${gameScore}`; setTimeout(() => { examCurrentIndex++; isGamePaused = false; loadExam2Question(); }, 800); } 
  else { playSound("wrong"); gameScore = Math.max(0, gameScore - 50); document.getElementById("exam2-score").innerText = `SCORE: ${gameScore}`; document.getElementById("exam2-input").classList.add("wrong"); setTimeout(() => { document.getElementById("exam2-input").classList.remove("wrong"); isGamePaused = false; document.getElementById("exam2-input").focus(); }, 800); }
});
